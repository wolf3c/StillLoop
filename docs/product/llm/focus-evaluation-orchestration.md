...
这种编排仍由 App 层 LLM gate 保持 1 并发，减少 helper 排队、并发调度和不必要的视觉推理，降低本地模型长尾风险。当前内置 llama.cpp runtime 暂停 prompt cache 实验，使用单 server slot；presence、alignment、progress、target judgment 和 review comment 仍在 App 层串行进入同一个 helper。代价是 aligned 场景不再重叠执行 presence/alignment，单轮墙钟耗时不一定总是低于两阶段并发。
