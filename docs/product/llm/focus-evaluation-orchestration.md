...
这种编排仍由 App 层 LLM gate 保持 1 并发，减少 helper 排队、并发调度和不必要的视觉推理，降低本地模型长尾风险。内置 llama.cpp runtime 可使用多个 server slot 主要是为了 prompt cache slot 复用，不表示 presence/alignment/progress 会并行请求；代价是 aligned 场景不再重叠执行 presence/alignment，单轮墙钟耗时不一定总是低于两阶段并发。
