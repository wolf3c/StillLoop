...
- `presence=away|resting`：直接合成 away/resting，不运行任务匹配或任务进展评估。
- `presence=present|unclear`：继续运行任务匹配评估。
- `alignment=unaligned|unclear`：直接合成 distracted/uncertain，不运行任务进展评估。

只有在 `presence=present|unclear` 且 `alignment=aligned` 时运行 `taskProgressOutcome`，用于区分 focused 和 stuck。

这种编排由 App 层 `LLMWorkScheduler` 保持“高层任务 FIFO + 任务内连续执行”。一轮 focus evaluation 会作为一个原子高层工作项进入队列，内部连续执行 `presence -> alignment -> progress`；排队中的 target judgment、review comment、readiness、prewarm 或 probe 不会插入到这一轮的三个子请求之间。

调度不是全局抢占式 priority queue。已经入队的前一轮 target judgment 不会被后一轮 focus evaluation 越过；同样，一旦某个 focus evaluation 开始执行，它的内部子请求会连续完成。未显式包进高层 work 的普通本地 LLM 调用仍会作为单请求 work 串行执行，避免绕过同一 runtime/helper。

当前内置 llama.cpp runtime 暂停 prompt cache 实验，使用单 server slot；presence、alignment、progress、target judgment 和 review comment 仍在 App 层串行进入同一个 helper。代价是 aligned 场景不再重叠执行 presence/alignment，单轮墙钟耗时不一定总是低于两阶段并发。

任务进展失败不会必然导致整轮失败，会被规范化为 unclear。用户在场或任务匹配失败时，编排会根据剩余结果决定是否能合成，或抛出拆分评估错误。被门控跳过的任务进展不是失败，不应记录伪造的 progress LLM usage/timings，也不应计入 combined metrics。
...
诊断日志会把这些字段拆成更适合分析的形式：

- 高层 work：`llmWorkKind`、`llmWorkSequence`、`llmQueueWaitMS`、`llmExecutionMS`。
- 子请求 wall time：`presenceLLMRequestMS`、`alignmentLLMRequestMS`、`progressLLMRequestMS`、`targetLLMRequestMS`。
- App 侧请求耗时：`*DurationMS` 现在表示实际 LLM 调用 wall time，不包含高层队列等待。
- 后端耗时：继续使用 `*PromptMS`、`*PredictedMS`、`*PromptN`、`*PredictedN`、`*CacheN`、`*CachedTokens`。
- token 统计优先来自 completion response 的 `usage`，记录为 `*PromptTokens`、`*CompletionTokens`、`*TotalTokens`；主链路不再为了 token 统计同步调用 `/tokenize`。

这些指标用于区分队列等待、App 请求墙钟、后端 prefill/output、缓存命中、图片数量和上下文大小。
