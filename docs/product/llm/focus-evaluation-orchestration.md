...
- `presence=away|resting`：直接合成 away/resting，不运行任务进展评估。
- `alignment=unaligned|unclear`：直接合成 distracted/uncertain，不运行任务进展评估。

第二阶段只在 `presence=present|unclear` 且 `alignment=aligned` 时运行 `taskProgressOutcome`，用于区分 focused 和 stuck。

这种编排减少非任务匹配场景的重型视觉推理，降低本地模型并发争用和长尾风险；代价是 aligned 场景可能多一个阶段，单轮墙钟耗时不一定总是低于三路并发。
