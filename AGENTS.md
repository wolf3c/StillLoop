...
- `2026-05-17`: Let nudge overlay tests confirm panel creation but not whether background-triggered panels remain visible above the user's active Space. Root cause: treated default status-bar-level panels as sufficient for real foreground visibility. Future rule: for overlay reminders, test global HUD level, cross-Space behavior, and manual visibility from a different foreground app before calling the reminder path done.
- `2026-05-17`: Shipped a `moveToActiveSpace` nudge-overlay fix that still did not make reminders visible. Root cause: guessed at Space movement instead of proving the panel's actual visibility level and cross-Space behavior. Future rule: when an overlay visibility fix fails once, escalate the window to an explicit global HUD behavior and verify the exact `NSWindow.Level` plus `collectionBehavior` in tests before relaunching.
