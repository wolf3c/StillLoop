...
The default built-in-model path uses the bundled llama.cpp helper under `Sources/StillLoop/Resources/Runtime/`. `scripts/run-app.sh` copies it into `.build/StillLoop.app/Contents/Helpers/stillloop-llama-server` with its `lib*.dylib` dependencies, signs the helper files, and StillLoop starts the helper lazily during a focus session. The built-in helper listens on a per-app Unix domain socket in the temporary directory, not a TCP localhost port, so Mac App Store builds do not need `com.apple.security.network.server`. If the focus screen shows a built-in runtime failure, check `pgrep -fl "stillloop-llama-server|llama-server|StillLoop"` before falling back to manual HTTP testing.
